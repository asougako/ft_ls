...
#define ERROR_ACC_STR (*(t_file_infos*)(*(t_list*)(*dirlst).content).content).error_str
void    print_dir(t_list *dirlst, size_t index)
{
    ...
    while (dirlst!= NULL)
    {
	if (index2 > 0 && ERROR_ACCESS == false)
	{
	    ft_putstr(DIRECTORY);
	    ft_putendl(":");
	}
	...
	dirlst = (*dirlst).next;
	if (dirlst != NULL && ERROR_ACCESS == false)
	    ft_putendl("");
	    index2++;
    }
}
...
#define	FILE_ERROR (*(t_file_infos*)(*link).content).error_access
#define	FILE_ERROR_STR (*(t_file_infos*)(*link).content).error_str
void	print_dir_long(t_list *file_lst)
{
	...
	while(link != NULL)
	{
		if (FILE_ERROR == true)
		{
			ft_putstr_fd(FILE_ERROR_STR, 2);
		}
		else if (FILE_PRINT == true)
		{
			if (OPT(i))
			{
